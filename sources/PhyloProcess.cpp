...
void PhyloProcess::CreateSuffStat()    {
    ...
    if (! siteratesuffstatcount)    {
        if (GetMyid())    {
            siteratesuffstatcount = new int[GetNsite()] __attribute__((aligned(512)));;
            siteratesuffstatbeta = new double[GetNsite()] __attribute__((aligned(512)));;
        }
    }
    if (! branchlengthsuffstatcount)    {
        branchlengthsuffstatcount = new int[GetNbranch()] __attribute__((aligned(512)));;
        branchlengthsuffstatbeta = new double[GetNbranch()] __attribute__((aligned(512)));;
    }
    activesuffstat = true;
}
...
void PhyloProcess::Create(Tree* intree, SequenceAlignment* indata,int indim)    {
    ...
    if (! data)    {
        ...
        empfreq = new double[data->GetNstate()] __attribute__((aligned(512)));;
        data->GetEmpiricalFreq(empfreq);

        loglarray = new double[GetNbranch()]    __attribute__((aligned(512)));;
        // MPI : slaves only
        ...
        if (myid > 0) {
            ...
            submap = new BranchSitePath**[GetNbranch()] __attribute__((aligned(512)));;
            for (int j=0; j<GetNbranch(); j++)    {
                submap[j] = 0;
            }
            nodestate = new int*[GetNnode()]     __attribute__((aligned(512)));
            condlmap = new double***[GetNlink()] __attribute__((aligned(512)));
            CreateNodeStates();
            CreateMissingMap();
            CreateMappings();
            condflag = false;
...
void PhyloProcess::CreateMissingMap()    {
    ...
    missingmap = new int*[GetNbranch()] __attribute__((aligned(512)));;
    for (int j=0; j<GetNnode(); j++)    {
        missingmap[j] = new int[GetNsite()] __attribute__((aligned(512)));;
...
void PhyloProcess::GlobalSetDataFromLeaves()    {
    ...
    MPI_Status stat;
    int* tmp = new int[maxwidth * GetNtaxa()] __attribute__((aligned(512)));;
...
void PhyloProcess::GlobalSetNodeStates()    {
    ...
    MPI_Status stat;
    int* tmp = new int[maxwidth * GetNnode()] __attribute__((aligned(512)));;
...
void PhyloProcess::SlaveSetDataFromLeaves()    {
    ...
    // mpi send the array
    int* tmp = new int[(sitemax - sitemin) * GetNtaxa()] __attribute__((aligned(512)));;
    int k = 0;
...
void PhyloProcess::GlobalUpdateBranchLengthSuffStat()    {
    ...
    double dvector[nbranch];
    for(i=1; i<nprocs; ++i) {
        MPI_Recv(ivector,nbranch,MPI_INT,MPI_ANY_SOURCE,10,MPI_COMM_WORLD,&stat);
        for(j=0; j<nbranch; ++j) {
    ...
    for(i=1; i<nprocs; ++i) {
        MPI_Recv(dvector,nbranch,MPI_DOUBLE,MPI_ANY_SOURCE,11,MPI_COMM_WORLD,&stat);
        for(j=0; j<nbranch; ++j) {
...
void PhyloProcess::SlaveUpdateBranchLengthSuffStat()    {
    ...
    MPI_Send(branchlengthsuffstatcount,GetNbranch(),MPI_INT,0,10,MPI_COMM_WORLD);
    // MPI_Barrier(MPI_COMM_WORLD);
    MPI_Send(branchlengthsuffstatbeta,GetNbranch(),MPI_DOUBLE,0,11,MPI_COMM_WORLD);
}
...
void PhyloProcess::GlobalGetMeanSiteRate()    {
    ...
    if (! meansiterate)    {
        meansiterate = new double[GetNsite()] __attribute__((aligned(512)));;
    }
...
void PhyloProcess::ReadSiteRates(string name, int burnin, int every, int until)    {
    ...
    double* meanrate = new double[GetNsite()] __attribute__((aligned(512)));;
    for (int i=0; i<GetNsite(); i++)    {
        meanrate[i] = 0;
    }
...
void PhyloProcess::AllPostPred(string name, int burnin, int every, int until, int inrateprior, int inprofileprior, int inrootprior)    {
    ...
    double* obstaxstat = new double[GetNtaxa()] __attribute__((aligned(512)));;
    ...
    double* meantaxstat = new double[GetNtaxa()] __attribute__((aligned(512)));;
    double* vartaxstat = new double[GetNtaxa()] __attribute__((aligned(512)));;
    double* pptaxstat = new double[GetNtaxa()] __attribute__((aligned(512)));;
    double* taxstat = new double[GetNtaxa()] __attribute__((aligned(512)));;
...
void PhyloProcess::ReadAncestral(string name, int burnin, int every, int until)    {
    ...
    double* allocmeanstatepostprob = new double[GetNsite()*GetNnode()*GetGlobalNstate()] __attribute__((aligned(512)));;
    double* allocstatepostprob = new double[GetNsite()*GetNnode()*GetGlobalNstate()] __attribute__((aligned(512)));;

    double*** meanstatepostprob = new double**[GetNsite()] __attribute__((aligned(512)));;
    double*** statepostprob = new double**[GetNsite()] __attribute__((aligned(512)));;
    for (int i=0; i<GetNsite(); i++)    {
        meanstatepostprob[i] = new double*[GetNnode()] __attribute__((aligned(512)));;
        statepostprob[i] = new double*[GetNnode()] __attribute__((aligned(512)));;
        for (int j=0; j<GetNnode(); j++)    {
...
void PhyloProcess::SlaveComputeStatePostProbs()    {
    ...
    // allocate arrays
    double* allocstatepostprob = new double[(GetSiteMax() - GetSiteMin())*GetNnode()*GetGlobalNstate()]  __attribute__((aligned(512)));;
    double*** statepostprob = new double**[GetNsite()] __attribute__((aligned(512)));;
    for (int i=GetSiteMin(); i<GetSiteMax(); i++)    {
        statepostprob[i] = new double*[GetNnode()] __attribute__((aligned(512)));;
        for (int j=0; j<GetNnode(); j++)    {
...
double PhyloProcess::GlobalGetFullLogLikelihood()  {
    ...
    double* tmp = new double[GetNsite()] __attribute__((aligned(512)));;
    double total = 0;
    for(int i=1; i<GetNprocs(); ++i) {
...
void PhyloProcess::ReadSiteLogL(string name, int burnin, int every, int until)    {
    ...
    double* tmp = new double[GetNsite()] __attribute__((aligned(512)));;
    double* mean = new double[GetNsite()] __attribute__((aligned(512)));;
    ...
    double meancpo = 0;
    double varcpo = 0;
    double* cpo = new double[GetNsite()] __attribute__((aligned(512)));;
    for (int i=0; i<GetNsite(); i++)    {
        double min = 0;
